 ...
 * @version $Date: 2014/03/04 12:02:33 $
 */
class CastFunction {
	
	
	/**
	 * Checks if T is an allowed type. Mixed, void and null are not allowed.
	 * Array must always indicate the key type (integer, string or both) and the
	...
	{
		// mixed, void and null are not allowed:
		if( $t === "mixed"
		|| Strings::endsWith($t, "]mixed")
		|| Strings::startsWith($t, "mixed[") )
			return "mixed not allowed in cast()";
		if( $t === "void"
		...
		// "array" and "array[...]" not allowed:
		if( $t === "array"
		|| Strings::startsWith($t, "array[") && Strings::endsWith($t, "]") )
			return "missing element type in array in cast()";

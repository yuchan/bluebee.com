...
# - the php.ini directory
# - the PHPLint program (should be stdlib/it/icosaedro/lint/PHPLint.php)
...
# Version: $Date: 2014/02/13 14:25:58 $
...
proc runPhplint { generate_doc } {
	...
	if { [string length $opts(php_lint)] == 0 } {
	...
	set cmd "[list $opts(php_cli) -c$opts(php_ini_dir) $opts(php_lint)] $opt"
...
proc SetDefault { } {
	...
	if { [file exists "stdlib/php.ini"] } {
		set opts(php_ini_dir) [abspath "stdlib"]
	} else {
		set opts(php_ini_dir) "MISSING_PHP_INI_DIRECTORY"
	}

	if { [file exists "stdlib/it/icosaedro/lint/PHPLint.php"] } {
		set opts(php_lint) [abspath "stdlib/it/icosaedro/lint/PHPLint.php"]
	} else {
		set opts(php_lint) ""
	}
...
proc Save { } {
	...
	if { [string length $opts(php_ini_dir)] > 0 } {
		save-str $fid opts(php_ini_dir) $opts(php_ini_dir)
	}
	if { [string length $opts(php_lint)] > 0 } {
		save-str $fid opts(php_lint) $opts(php_lint)
	}
	if { [string length $opts(modules_dir)] > 0 } {
...
proc Help { } {
	...
	.report.text insert 1.0 "phplint.tcl - graphical interface to the PHPLint program

Usage: phplint.tcl \[file]
...
Configuration: PHP CLI executable
---------------------------------
The PHP CLI (command-line interface) version of the PHP interpreter.

Configuration: php.ini directory
--------------------------------
Directory of the php.ini file. Should be the stdlib directory because
stdlib/autoload.php requires this directory by default.

Configuration: PHPLint program
------------------------------
The PHPLint program, normally stdlib/it/icosaedro/lint/PHPLint.php.

Configuration: Modules directory
--------------------------------
The dir. of the modules, normally \"modules\". You may also indicate a
list of directories separated by \":\" (Unix, Linux) or by \";\" (Windows).

Configuration: Source editor program
------------------------------------
The command indicated there is invoked with the PHP source as argument when
the button \[Edit] is pressed.
...
Doc. charset
------------
Encoding set in the header of the HTML pages generated from the PHPLint
documentor. The encoding should match the encoding used for the DocBlock
comments.
...
set f $cfg_panel.php_ini_dir
frame $f
label $f.l -text "php.ini directory:" -font $font_norm
entry $f.v -textvariable opts(php_ini_dir) -background white -width 80
button $f.sel -text "..." -command {
	global opts
	set new $opts(php_ini_dir)
	set new [tk_chooseDirectory \
		-initialdir $opts(php_ini_dir) \
		-parent . \
		-title "Select the php.ini directory" ]
	if { [string length $new] > 0 } {
		if { [string length $opts(php_ini_dir)] == 0 } {
			set opts(php_ini_dir) $new
		} else {
			set opts(php_ini_dir) "$opts(php_ini_dir):$new"
		}
	}
}
pack $f -side top -anchor nw -fill x
...
entry $f.v -textvariable opts(php_lint) -background white -width 80
button $f.sel -text "..." -command {
	global opts
	set new $opts(php_lint)
	set types {
		{{PHP}       {.php} }
		{{All Files}  *     }
	}
	set new [tk_getOpenFile \
		-initialfile $opts(php_lint) \
		-parent . \
		-filetypes $types \
		-title "Select the PHPLint program" ]
	if { [string length $new] > 0 } {
		if { [string length $opts(php_lint)] == 0 } {
			set opts(php_lint) $new
		} else {
			set opts(php_lint) "$opts(php_lint):$new"
		}
	}
}
pack $f -side top -anchor nw -fill x

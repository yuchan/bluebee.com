...
$notifications_edit = NULL; // Initialize page object first

class cnotifications_edit extends cnotifications {
	...
	// Page object name
	var $PageObjName = 'notifications_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("notificationslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// object_type
			$this->object_type->SetDbValueDef($rsnew, $this->object_type->CurrentValue, NULL, $this->object_type->ReadOnly);

			// object_id
			$this->object_id->SetDbValueDef($rsnew, $this->object_id->CurrentValue, NULL, $this->object_id->ReadOnly);

			// possessive
			$this->possessive->SetDbValueDef($rsnew, $this->possessive->CurrentValue, NULL, $this->possessive->ReadOnly);

			// from_user_id
			$this->from_user_id->SetDbValueDef($rsnew, $this->from_user_id->CurrentValue, NULL, $this->from_user_id->ReadOnly);

			// clicked
			$this->clicked->SetDbValueDef($rsnew, $this->clicked->CurrentValue, NULL, $this->clicked->ReadOnly);

			// relevant_id
			$this->relevant_id->SetDbValueDef($rsnew, $this->relevant_id->CurrentValue, NULL, $this->relevant_id->ReadOnly);

			// relevant_object
			$this->relevant_object->SetDbValueDef($rsnew, $this->relevant_object->CurrentValue, NULL, $this->relevant_object->ReadOnly);
...
// Create page object
if (!isset($notifications_edit)) $notifications_edit = new cnotifications_edit();

// Page init
$notifications_edit->Page_Init();

// Page main
$notifications_edit->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var notifications_edit = new ew_Page("notifications_edit");
notifications_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = notifications_edit.PageID; // For backward compatibility

// Form object
var fnotificationsedit = new ew_Form("fnotificationsedit");

// Validate form
fnotificationsedit.Validate = function(fobj) {
...
fnotificationsedit.Form_CustomValidate =  
...
fnotificationsedit.ValidateRequired = true;
<?php } else { ?>
fnotificationsedit.ValidateRequired = false; 
<?php } ?>
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $notifications->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $notifications->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $notifications_edit->ShowPageHeader(); ?>
<?php
$notifications_edit->ShowMessage();
?>
<form name="fnotificationsedit" id="fnotificationsedit" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="notifications">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_notificationsedit" class="ewTable">
...
<?php echo $notifications->id->EditValue ?></span>
<input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($notifications->id->CurrentValue) ?>">
</span><?php echo $notifications->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fnotificationsedit.Init();
</script>
<?php
$notifications_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$notifications_edit->Page_Terminate();
?>

...
$tbl_subject_group = NULL;

//
// Table class for tbl_subject_group
//
class ctbl_subject_group extends cTable {
	var $id;
	var $subject_type_id;
	var $faculty_id;
	var $dept_id;
	var $subject_group_name;
	var $subject_group_info;
	...
	function __construct() {
		...
		$this->TableVar = 'tbl_subject_group';
		$this->TableName = 'tbl_subject_group';
		...
		// id
		$this->id = new cField('tbl_subject_group', 'tbl_subject_group', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// subject_type_id
		$this->subject_type_id = new cField('tbl_subject_group', 'tbl_subject_group', 'x_subject_type_id', 'subject_type_id', '`subject_type_id`', '`subject_type_id`', 3, -1, FALSE, '`subject_type_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->subject_type_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['subject_type_id'] = &$this->subject_type_id;

		// faculty_id
		$this->faculty_id = new cField('tbl_subject_group', 'tbl_subject_group', 'x_faculty_id', 'faculty_id', '`faculty_id`', '`faculty_id`', 3, -1, FALSE, '`faculty_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->faculty_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['faculty_id'] = &$this->faculty_id;

		// dept_id
		$this->dept_id = new cField('tbl_subject_group', 'tbl_subject_group', 'x_dept_id', 'dept_id', '`dept_id`', '`dept_id`', 3, -1, FALSE, '`dept_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->dept_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['dept_id'] = &$this->dept_id;

		// subject_group_name
		$this->subject_group_name = new cField('tbl_subject_group', 'tbl_subject_group', 'x_subject_group_name', 'subject_group_name', '`subject_group_name`', '`subject_group_name`', 200, -1, FALSE, '`subject_group_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['subject_group_name'] = &$this->subject_group_name;

		// subject_group_info
		$this->subject_group_info = new cField('tbl_subject_group', 'tbl_subject_group', 'x_subject_group_info', 'subject_group_info', '`subject_group_info`', '`subject_group_info`', 201, -1, FALSE, '`subject_group_info`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['subject_group_info'] = &$this->subject_group_info;
	}
	...
	// Update Table
	var $UpdateTable = "`tbl_subject_group`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "tbl_subject_grouplist.php";
		}
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("tbl_subject_groupview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "tbl_subject_groupadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("tbl_subject_groupedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("tbl_subject_groupadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tbl_subject_groupdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->faculty_id->Exportable) $Doc->ExportCaption($this->faculty_id);
				if ($this->dept_id->Exportable) $Doc->ExportCaption($this->dept_id);
				if ($this->subject_group_name->Exportable) $Doc->ExportCaption($this->subject_group_name);
				if ($this->subject_group_info->Exportable) $Doc->ExportCaption($this->subject_group_info);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->subject_type_id->Exportable) $Doc->ExportCaption($this->subject_type_id);
				if ($this->faculty_id->Exportable) $Doc->ExportCaption($this->faculty_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->faculty_id->Exportable) $Doc->ExportField($this->faculty_id);
					if ($this->dept_id->Exportable) $Doc->ExportField($this->dept_id);
					if ($this->subject_group_name->Exportable) $Doc->ExportField($this->subject_group_name);
					if ($this->subject_group_info->Exportable) $Doc->ExportField($this->subject_group_info);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->subject_type_id->Exportable) $Doc->ExportField($this->subject_type_id);
					if ($this->faculty_id->Exportable) $Doc->ExportField($this->faculty_id);

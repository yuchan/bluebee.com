...
$tbl_subject_faculty = NULL;

//
// Table class for tbl_subject_faculty
//
class ctbl_subject_faculty extends cTable {
	var $id;
	var $subject_id;
	var $faculty_id;
	var $is_active;
	...
	function __construct() {
		...
		$this->TableVar = 'tbl_subject_faculty';
		$this->TableName = 'tbl_subject_faculty';
		...
		// id
		$this->id = new cField('tbl_subject_faculty', 'tbl_subject_faculty', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// subject_id
		$this->subject_id = new cField('tbl_subject_faculty', 'tbl_subject_faculty', 'x_subject_id', 'subject_id', '`subject_id`', '`subject_id`', 3, -1, FALSE, '`subject_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->subject_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['subject_id'] = &$this->subject_id;

		// faculty_id
		$this->faculty_id = new cField('tbl_subject_faculty', 'tbl_subject_faculty', 'x_faculty_id', 'faculty_id', '`faculty_id`', '`faculty_id`', 3, -1, FALSE, '`faculty_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->faculty_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['faculty_id'] = &$this->faculty_id;

		// is_active
		$this->is_active = new cField('tbl_subject_faculty', 'tbl_subject_faculty', 'x_is_active', 'is_active', '`is_active`', '`is_active`', 3, -1, FALSE, '`is_active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->is_active->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['is_active'] = &$this->is_active;

		// university_id
		$this->university_id = new cField('tbl_subject_faculty', 'tbl_subject_faculty', 'x_university_id', 'university_id', '`university_id`', '`university_id`', 3, -1, FALSE, '`university_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->university_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['university_id'] = &$this->university_id;
	}
	...
	// Update Table
	var $UpdateTable = "`tbl_subject_faculty`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "tbl_subject_facultylist.php";
		}
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("tbl_subject_facultyview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "tbl_subject_facultyadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("tbl_subject_facultyedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("tbl_subject_facultyadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tbl_subject_facultydelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->university_id->Exportable) $Doc->ExportCaption($this->university_id);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->subject_id->Exportable) $Doc->ExportCaption($this->subject_id);
				if ($this->faculty_id->Exportable) $Doc->ExportCaption($this->faculty_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->university_id->Exportable) $Doc->ExportField($this->university_id);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->subject_id->Exportable) $Doc->ExportField($this->subject_id);
					if ($this->faculty_id->Exportable) $Doc->ExportField($this->faculty_id);

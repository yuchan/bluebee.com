...
//
class ctbl_faculty extends cTable {
	var $faculty_id;
	var $faculty_university;
	var $faculty_name;
	var $faculty_code;
	var $faculty_active;
	var $faculty_research;
	var $faculty_lab;
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('faculty_id') . '=' . ew_QuotedValue($rs['faculty_id'], $this->faculty_id->FldDataType) . ' AND ';
		}
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@faculty_id@", ew_AdjustSql($this->faculty_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->faculty_university->Exportable) $Doc->ExportCaption($this->faculty_university);
				if ($this->faculty_name->Exportable) $Doc->ExportCaption($this->faculty_name);
				if ($this->faculty_code->Exportable) $Doc->ExportCaption($this->faculty_code);
				if ($this->faculty_active->Exportable) $Doc->ExportCaption($this->faculty_active);
				if ($this->faculty_research->Exportable) $Doc->ExportCaption($this->faculty_research);
				if ($this->faculty_lab->Exportable) $Doc->ExportCaption($this->faculty_lab);
			} else {
				if ($this->faculty_id->Exportable) $Doc->ExportCaption($this->faculty_id);
				if ($this->faculty_university->Exportable) $Doc->ExportCaption($this->faculty_university);
				if ($this->faculty_name->Exportable) $Doc->ExportCaption($this->faculty_name);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->faculty_university->Exportable) $Doc->ExportField($this->faculty_university);
					if ($this->faculty_name->Exportable) $Doc->ExportField($this->faculty_name);
					if ($this->faculty_code->Exportable) $Doc->ExportField($this->faculty_code);
					if ($this->faculty_active->Exportable) $Doc->ExportField($this->faculty_active);
					if ($this->faculty_research->Exportable) $Doc->ExportField($this->faculty_research);
					if ($this->faculty_lab->Exportable) $Doc->ExportField($this->faculty_lab);
				} else {
					if ($this->faculty_id->Exportable) $Doc->ExportField($this->faculty_id);
					if ($this->faculty_university->Exportable) $Doc->ExportField($this->faculty_university);
					if ($this->faculty_name->Exportable) $Doc->ExportField($this->faculty_name);
